Hooks.on('renderAbilityUseDialog', function(options) {
    if($('.dnd5e.dialog #ability-use-form select[name="slotLevel"]').length > 0) { // If the dialog box has a option to select a spell level
        ...
        $(options._element[0]).find(`select[name="slotLevel"] option`).each(function() {
